...
\begin{multicols}{2}
	...
	\vskip 0.1cm
		...
		\begin{figure}[H]
			...
			\resizebox{\columnwidth}{!}{\begin{tikzpicture}[node distance={15mm}, thick, main/.style = {draw, circle}] 
				...
				\draw (13) -- (14);
			\end{tikzpicture}}
			\caption{Hệ thống điểm quan sát với khoảng cách quan sát lớn nhất $140$. Mỗi điểm $A_i^{\bullet}$ thuộc tập $M_i$ có thể quan sát được tất cả điểm trong $M_{i-1}, M_{i+1}$ cũng như các điểm còn lại trong $M_i$.}
			\vspace*{-10pt}
		\end{figure}
		\vskip 0.1cm		
		$\bullet$ Mỗi tập $M_{3k}$ và $M_{3k+2}$ với $k = 0, \ldots, 46$ chỉ chứa một điểm quan sát.
		\vskip 0.1cm
		$\bullet$ Mỗi tập $M_{1}$ và $M_{139}$ chứa $42$ điểm quan sát.
		\vskip 0.1cm
		$\bullet$ Mỗi tập $M_{3k+1}$ với $k = 1, \ldots, 45$ có $41$ điểm quan sát.
		\vskip 0.1cm
		Hệ thống này thỏa mãn điều kiện rằng mọi điểm quan sát trong $M_i$ với $i = 1, \ldots, 139$ quan sát được tât cả điểm còn lại trong $M_{i}$ cũng như tất cả các điểm trong tập đứng trước $M_{i-1}$ và tất cả các điểm trong tập đứng sau $M_{i+1}$. Điểm quan sát duy nhất trong $M_0$ quan sát được $42$ điểm trong $M_1$ và điểm quan sát duy nhất trong $M_{140}$ quan sát được $42$ điểm trong $M_{139}$ (Hình $1$).
		\vskip 0.1cm
		Rõ ràng $M_i \neq \emptyset$ với mọi $i = 0, \ldots, 140$ và tổng số điểm quan sát là
		\begin{align*}
...
%		$"\Leftarrow":$
		...
		\begin{figure}[H]
			...
			\end{tikzpicture}}
			\caption{Đa giác đều $16$ cạnh.}
			\vspace*{-10pt}
		\end{figure}
		\underline{\textit{Trường hợp $n \equiv 1 \mod 4$.}} Đặt $n = 4k+1$. 
		\vskip 0.1cm
		$\bullet$ Với $k=1$ ($2n = 10$) ta có thể tô màu như Hình $4$.
		\begin{figure}[H]
			\vspace*{-5pt}
			\centering
			\captionsetup{labelformat= empty, justification=centering}
			...
			\end{tikzpicture}}
			\caption{Đa giác đều $10$ cạnh.}
			\vspace*{-10pt}
		\end{figure}
		$\bullet$  Với $k \ge 2$ thì danh sách các cạnh được tô màu đỏ cùng với khoảng cách giữa các đỉnh (phương trình ($4$) với $2n = 8k+2$) được cho trong bảng dưới đây:
		\begin{center}
			\resizebox{\linewidth}{!}{%
				\begin{tabular}{ |c|c|c| } 
					\hline
		...
		\begin{figure}[H]
			...
			\resizebox{\columnwidth}{!}{\begin{tikzpicture}
				...
				{--++(360+\w/2-\i*\w+\w:\laenge) coordinate (A\i)}
				--cycle;
				\tkzDrawPoints(A1,A...,A26)
				\foreach \i in {1,...,\n}\node[anchor={270-\i*\w +\w}, circle] at (A\i){$A_{\i}$};
				\tkzDrawSegment[red](A1,A26)
				\tkzDrawSegment[red](A2,A25)
				\tkzDrawSegment[red](A3,A24)
				\tkzDrawSegment[red](A4,A18)
				\tkzDrawSegment[red](A5,A23)
				\tkzDrawSegment[red](A6,A22)
				\tkzDrawSegment[red](A7,A14)
				\tkzDrawSegment[red](A8,A21)
				\tkzDrawSegment[red](A9,A20)
				\tkzDrawSegment[red](A10,A19)
				\tkzDrawSegment[red](A11,A17)
				\tkzDrawSegment[red](A12,A16)
				\tkzDrawSegment[red](A13,A15)
			\end{tikzpicture}}
			\caption{Đa giác đều $26$ cạnh.}
			\vspace*{-10pt}
		\end{figure}
\end{multicols}